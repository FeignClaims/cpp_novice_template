...
class Widget {
 ...
 public:
  Widget(Point xy, int w, int h, string const& s, Callback cb) : loc(xy), width(w), height(h), label(s), do_it(cb) {}
...
class Button : public Widget {
 public:
  Button(Point xy, int ww, int hh, string const& s, Callback cb) : Widget(xy, ww, hh, s, cb) {}
  void attach(Window& win);
};

struct In_box : Widget {
  In_box(Point xy, int w, int h, string const& s) : Widget(xy, w, h, s, 0) {}
  int get_int();
  string get_string();
...
struct Out_box : Widget {
  Out_box(Point xy, int w, int h, string const& s /*, Window& win*/) : Widget(xy, w, h, s, 0) {}
  void put(int);
...
struct Menu : Widget {
  ...
  Menu(Point xy, int w, int h, Kind kk, string const& s);
