...
struct Color {
  ...
  Color(Color_type cc) : v(visible), c(static_cast<Fl_Color>(cc)) {}
  Color(Color_type cc, Transparency vv) : v(vv), c(static_cast<Fl_Color>(cc)) {}
  Color(int cc) : v(visible), c(static_cast<Fl_Color>(cc)) {}
...
struct Text : Shape {
  ...
  Text(Point x, std::string const& s) : lab{s} {
    add(x);
  }
...
struct Ellipse : Shape {
  ...
  Point focus1() const {
    return {center().x + static_cast<int>(sqrt(static_cast<double>(w * w - h * h))), center().y};
  }
  Point focus2() const {
    return {center().x - static_cast<int>(sqrt(static_cast<double>(w * w - h * h))), center().y};
  }
...
struct Marked_polyline : Open_polyline {
  Marked_polyline(std::string const& m) : mark(m) {}
  void draw_lines() const;
...
struct Marks : Marked_polyline {
  Marks(std::string const& m) : Marked_polyline(m) {
